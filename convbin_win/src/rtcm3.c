                         ...
                         const int *ex, const int *half)
{
    ...
    double tt,freq;
    uint8_t code[32];
    char *msm_type="",*q=NULL, *msg, sat_str[32];
    ...
    for (i=j=0;i<h->nsat;i++) {
        ...
        if (rtcm->outtype) {
            msg = rtcm->msgtype + strlen(rtcm->msgtype);
            satno2id(sat, sat_str);
            sprintf(msg, "%s  ", sat_str);
        }
        ...
        for (k=0;k<h->nsig;k++) {
            ...
            if (sat&&index>=0&&idx[k]>=0) {
                ...
                if (r[i]!=0.0&&pr[j]>-1E12) {
                    ...
                    if (rtcm->outtype) {
                        msg = rtcm->msgtype + strlen(rtcm->msgtype);
                        sprintf(msg, "%10.4f C%s ",rtcm->obs.data[index].P[idx[k]], sig[k]);
                    }
                }
                /* carrier-phase (cycle) */
                if (r[i]!=0.0&&cp[j]>-1E12) {
                    ...
                    if (rtcm->outtype) {
                        msg = rtcm->msgtype + strlen(rtcm->msgtype);
                        sprintf(msg, "%10.4f L%s \t",rtcm->obs.data[index].L[idx[k]], sig[k]);
                    }
                }
                /* doppler (hz) */
                if (rr&&rrf&&rrf[j]>-1E12) {
                    ...
                    if (rtcm->outtype) {
                        msg = rtcm->msgtype + strlen(rtcm->msgtype);
                        sprintf(msg, "%10.4f ",rtcm->obs.data[index].D[idx[k]]);
                    }
                }
